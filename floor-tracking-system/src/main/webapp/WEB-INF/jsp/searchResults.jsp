...
<c:if test="${!empty associateList}">
	...
	<div class="hz-scroll-outer" style="height: 40%;">
		<table class="tablesorter">
			...
			<tbody>
				<c:forEach var="anAssociate" items="${associateList}"
					varStatus="status">
...
<div id="buttonsPanel" class="center">
	<c:if test="${showExport eq true }">
		<form action="./exportResults.ats" method="post" id="exportForm">
			<input type="hidden" id="exportdata" name="data" /> <input
				type="button" class="g-button" id="exportSearch"
				value="Export Results" />
		</form>
	</c:if>
	<c:if test="${allocateNew eq true }">
		<input type="submit" class="g-button" id="addNewEmp"
			value="Allocate New" onclick="showAdd('${cubeId }')">
	</c:if>
	<input type="button" class="g-button" value="Another Search"
		id="anotherSrchBtn" />
</div>
